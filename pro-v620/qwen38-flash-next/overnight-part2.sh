...
s6_restore() {
  ...
  note "- CT 121 (Hermes) **started**. ✅ Its cron CATCHES UP a missed job on startup — the"
  note "  22:00 backup committed 4 s after the container came back, so it does not need a"
  note "  manual trigger. ⚠️ But any OTHER catch-up job fires at the same moment and will"
  note "  contend with whatever is still benchmarking: the KB Freshness Refresh ran for 53 min,"
  note "  staged 0 of 448 entries and quarantined 5 as runaways, because it was calling CT 120"
  note "  while a sweep was restarting it. Check and un-quarantine after a benchmark night."
